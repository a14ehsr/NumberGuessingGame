...
### ゲーム設定の変更
- `-game 1000`などと追加で入力を行うとゲーム数を指定することができます.  
オプションをつけない場合の既定値は100です．
- `-round roundNum` : 最大ラウンド数を`roundNum`に指定します．規定値は100です．
- `-c changeLimit`  : 防御側が1度数値を変更してから再度変更できるまでの制限回数を`changeLimit`に変更します．規定値は10です．
- `-min minNum`     : 使用する数値の最小値を`minNum`に変更します．規定値は1です．
- `-max maxNum`     : 使用する数値の最大値を`maxNum`に変更します．規定値は100です．

### 対戦結果の出力について
`-olevel 2`などと追加で入力を行うと出力のモードを変更することができます．  
結果出力レベルは以下の通りです．  
> 1:当てた回数と当てるまでの平均回数(既定)   
> 2:モード1の出力に加え，各ゲームでの結果  
> 3:モード2の出力に加え，各ラウンドの数値
